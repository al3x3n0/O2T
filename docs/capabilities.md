...
| `freeze` (poison laundering) | ✅ verifies introduction (Track B) | a pass INTRODUCING `freeze` is verified (its choice is existential). REMOVING one is decided only when the operand is provably free of BOTH poison and undef (i.e. built from `noundef` params/constants), where the universal quantifier has a one-element domain; otherwise it declines, because the identity shortcut is a false proof reference Alive2 refutes. **Measured lift on LLVM's own tests: zero** — their freeze tests use possibly-`undef` parameters, which is exactly the case this cannot decide |
