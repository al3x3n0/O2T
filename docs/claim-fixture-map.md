...
| Structured-tree front-end: pre-parsed matcher/rewrite trees recover the IDENTICAL obligation, removing the tokenizer/parser from the TCB -- a Clang-AST producer (`clang -ast-dump=json`, STUB-mode) realizes this byte-for-byte on stub-compatible folds (unguarded/guarded/return-form); verbatim-upstream reach needs real headers + compile context (0/8 today, stated in maturity.md) | `pass_graph_structured_fixture`, `clang_tree_fixture` |
