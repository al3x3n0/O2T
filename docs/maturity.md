...
- **Closed-loop translation validation** (E1): proving *real* `opt` output equivalent to its input
  ...
  **peephole analogue** now exists too (`observe_fold_fixture`): a fold recovered from source is
  ...
  refinement is proved. Measured over `and/or/xor/add.ll` — **511/715 functions (71%) proved sound
  end-to-end, 0 false refutations, 0 of them vacuous**; the rest decline (unmodeled vocabulary, vector
  and memory shapes, a few timing-dependent z3-timeouts). The scalar refinement path accounts for ~430
  and the memory/vector dispatch for the other ~81. Every proof is additionally probed for **vacuity** (a
  ...
  whole-pass) end-to-end result whose reach (71%) far exceeds source-recovery (Track A, ~4%) because
