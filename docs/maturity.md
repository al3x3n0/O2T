...
1. **Clang-AST front-end: broaden verbatim reach** (highest leverage). Both modes wired
   ...
   name contract, predicate-set splits). Remaining: (a) the last string-path shapes not yet on the
   AST are the operand/reduction collapse LOOPS (phases 34-35); and (b) parse whole upstream `.cpp`s
   in their lib-internal compile context (compile DB) so every fold's real method resolves, not just
