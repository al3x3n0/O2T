...
1. **Clang-AST front-end: broaden verbatim reach** (highest leverage). Both modes wired
   ...
   name contract). Remaining: (a) extend the extractor's fold-shape coverage on the real AST to the
   last string-path shapes (predicate-set splits; the operand/reduction loops), and (b) parse whole
   upstream `.cpp`s in their lib-internal compile context (compile DB) so every fold's real method
   resolves, not just self-contained free functions.
