...
| 40 | The TWO-ICMP caller contract: `foldX(ICmpInst *Cmp0, ICmpInst *Cmp1, bool IsAnd, ...)` -- the caller guarantees the replaced value is the IsAnd-selected combination of the two cmps. Two-primary composition under a case-selected combiner; per-case `IsAnd`/`!IsAnd` reachability filters; rewrite-side operand PROJECTION (`Cmp0->getOperand(J)` lowers to the matched subtree). VERBATIM `foldIsPowerOf2OrZero` proves BOTH arms (real ctpop theorems via the phase-26 model); UGE-mutation and combiner-swap teeth refute (`_two_icmp_arm`, `projections` in `recover_pair_cases`). E6: 10 -> 12 | **Done** (same module) |
