#!/usr/bin/env python3
"""Generate and run CBMC/ESBMC harnesses for source-mined memory transforms."""
...
def main() -> int:
    ...
    parser.add_argument("--widths", default="native",
                        help="native, or comma-separated memory widths such as 8,16,32,64")
    parser.add_argument("--summary-json", type=Path)
    parser.add_argument("--report", type=Path, help="Alias for --summary-json")
    args = parser.parse_args()

    summary = run_memory_source_modelcheck(
        args.source,
        args.out_dir,
        args.engine,
        args.unwind,
    ...
    for result in summary.get("results", []):
        ...
        print(f"  [{result.get('status'):11}] {source_function}{width}{domain}{suffix}", file=sys.stderr)
