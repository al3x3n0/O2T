#!/usr/bin/env python3
"""Generate and run CBMC/ESBMC harnesses for source-mined SLP folds."""
...
from o2t.symexec.modelcheck_slp import run_slp_source_modelcheck  # noqa: E402
...
def main() -> int:
    ...
    summary = run_slp_source_modelcheck(
        args.source,
        args.out_dir,
        args.engine,
        args.unwind,
    ...
    for result in summary.get("results", []):
        ...
        detail = f" {result.get('status_detail')}" if result.get("status_detail") else ""
        suffix = f" ({result.get('reason')})" if result.get("reason") else ""
        print(f"  [{result.get('status'):11}] {source_function}{width}{domain}{detail}{suffix}", file=sys.stderr)
