#!/usr/bin/env python3
"""Discharge the deep GlobalOpt dead-initializer contracts (semantic, not syntactic).

Proves that defaulting an internal global's initializer to null preserves every observable
load, with two-sided teeth: a read-before-store, a read-only global, and an external-linkage
global are each REFUTED with a concrete `init != 0` witness. Needs z3.
"""
...
from o2t.validate.globalopt_model import run_contracts  # noqa: E402
