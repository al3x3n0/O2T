...
Runs the actual `opt -passes=instcombine` on a `.ll`, translates the before and after of each
single-BB integer function to an SMT term for its returned value, and proves them equal for all
inputs (QF_BV) -- so the proof is about the instructions InstCombine really emitted. A function
using an unmodeled instruction is soundly declined (`unsupported`), never falsely proved. Needs
z3 and opt.
"""
...
DEFAULT_SOURCE = ROOT / "tests" / "fixtures" / "instcombine_ir_cases.ll"
...
def main() -> int:
    ...
    opt_text = scalar_ir.run_instcombine(src, opt)
    if opt_text is None:
        print(json.dumps({"status": "error", "reason": "opt -passes=instcombine failed"}))
        return 1

    results = [scalar_ir.validate_instcombine(z3, src, opt_text, fn)
