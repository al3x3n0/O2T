...
Value *loadStorePack(Value **Base, Value *V, IRBuilder &Builder) {
  Value *L0 = Builder.CreateLoad(Base[0]);
  Builder.CreateStore(V, Base[1]);
...
void vectorizeMemoryStoreTree(TreeEntry &Entry, Value **A, Value *V,
                              IRBuilder &Builder, TargetTransformInfo &TTI) {
  if (allSameOpcode(Entry, Instruction::Add) && isValidElementType(Entry) &&
      isProfitable(Entry, TTI)) {
    Value *LHS = loadStorePack(A, V, Builder);
