...
Value *loadAliasAnalysisGuardMemory(Value **Base, IRBuilder &Builder) {
...
void vectorizeAliasAnalysisGuardMemoryTree(TreeEntry &Entry, Value **In,
                                           Value **Out, IRBuilder &Builder,
                                           AliasAnalysis *AA,
                                           TargetTransformInfo &TTI) {
  if (allSameOpcode(Entry, Instruction::Add) && isValidElementType(Entry) &&
      AA->isNoAlias(Out, In) && isProfitable(Entry, TTI)) {
    Value *LHS = loadAliasAnalysisGuardMemory(In, Builder);
