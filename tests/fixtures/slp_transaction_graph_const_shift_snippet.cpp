...
void vectorizeConstShiftTree(TreeEntry &Entry, IRBuilder &Builder,
                             TargetTransformInfo &TTI) {
  if (allSameOpcode(Entry, Instruction::Add) && isValidElementType(Entry) &&
      isProfitable(Entry, TTI)) {
    Value *LHS = packOperand(Entry, 0);
    Value *RHS = packOperand(Entry, 1);
    Value *ShiftAmount = ConstantInt::get(Type::getInt32Ty(), 3);
    Value *VectorAdd = Builder.CreateAdd(LHS, RHS);
    Value *VectorShift = Builder.CreateShl(VectorAdd, ShiftAmount);
