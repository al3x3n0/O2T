...
Value *loadContiguousMemory(Value **Base, IRBuilder &Builder) {
...
void vectorizeLoadStoreMemoryTree(TreeEntry &Entry, Value **In, Value **Out,
                                  IRBuilder &Builder,
                                  TargetTransformInfo &TTI) {
  if (allSameOpcode(Entry, Instruction::Add) && isValidElementType(Entry) &&
      noAlias(In, Out) &&
      isProfitable(Entry, TTI)) {
    Value *LHS = loadContiguousMemory(In, Builder);
