...
Value *loadGuardedMaskedMemory(Value **Base, Value **Mask, Value **Passthru,
                               IRBuilder &Builder) {
  Value *L0 = Passthru[0];
  if (Mask[0]) {
...
void vectorizeGuardedMaskedLoadStoreMemoryTree(TreeEntry &Entry, Value **In,
  ...
  if (allSameOpcode(Entry, Instruction::Add) && isValidElementType(Entry) &&
    ...
    Value *LHS = loadGuardedMaskedMemory(In, Mask, Passthru, Builder);
