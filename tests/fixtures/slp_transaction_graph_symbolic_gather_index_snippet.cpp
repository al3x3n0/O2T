...
Value *loadSymbolicGatherPack(Value **Base, int Lane, IRBuilder &Builder) {
...
void vectorizeSymbolicGatherTree(TreeEntry &Entry, Value **In, int Lane,
                                 IRBuilder &Builder,
                                 TargetTransformInfo &TTI) {
  if (allSameOpcode(Entry, Instruction::Add) && isValidElementType(Entry) &&
      isProfitable(Entry, TTI)) {
    Value *LHS = loadSymbolicGatherPack(In, Lane, Builder);
