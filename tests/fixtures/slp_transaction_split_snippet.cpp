...
void vectorizeTree(TreeEntry &Entry, IRBuilder &Builder, TargetTransformInfo &TTI) {
  if (discoverCandidate(Entry) && checkLegality(Entry) && isProfitable(Entry, TTI)) {
    Value *VectorResult = emitVectorOp(Entry, Builder);
    replaceExternalUses(Entry, VectorResult);
  }
}
