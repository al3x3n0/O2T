...
Value *loadIncompleteScalableTupleMaskedMemory(Value **Base, Value **Cmp,
                                               Value **Passthru,
                                               IRBuilder &Builder) {
  ...
  Value *T2 = Builder.CreateICmp(CmpInst::ICMP_NE, Cmp[2], Base[2]);
  Value *M2;
  if (C2) {
    M2 = T2;
  }
...
void vectorizeIncompleteScalableTupleMaskedMemoryTree(
  ...
  if (allSameOpcode(Entry, Instruction::Add) && isValidElementType(Entry) &&
    ...
    Value *LHS =
        loadIncompleteScalableTupleMaskedMemory(In, Cmp, Passthru, Builder);
    ...
    Value *S2C = Builder.CreateICmp(CmpInst::ICMP_EQ, Cmp[2], Passthru[2]);
    Value *S2T = Builder.CreateICmp(CmpInst::ICMP_NE, Cmp[2], In[2]);
    Value *S2;
    if (S2C) {
      S2 = S2T;
    }
