...
Value *loadDuplicateGatherPack(Value **Base, IRBuilder &Builder) {
...
void vectorizeDuplicateGatherTree(TreeEntry &Entry, Value **A,
                                  IRBuilder &Builder,
                                  TargetTransformInfo &TTI) {
  if (allSameOpcode(Entry, Instruction::Add) && isValidElementType(Entry) &&
      isProfitable(Entry, TTI)) {
    Value *LHS = loadDuplicateGatherPack(A, Builder);
