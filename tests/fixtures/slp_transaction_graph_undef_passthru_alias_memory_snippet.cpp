...
Value *loadUndefPassthruAliasMemory(Value **Base, Value **Mask,
                                    IRBuilder &Builder) {
  Value *P0 = UndefValue::get(Base[0]);
  Value *L0 = Builder.CreateMaskedLoad(Base[0], Mask[0], P0);
  Value *P1 = PoisonValue::get(Base[1]);
  Value *Q1 = P1;
  Value *L1 = Builder.CreateMaskedLoad(Base[1], Mask[1], Q1);
  Value *P2 = nullptr;
  Value *L2 = Builder.CreateMaskedLoad(Base[2], Mask[2], P2);
  Value *P3 = UndefValue::get(Base[3]);
  Value *Q3 = P3;
  Value *L3 = Builder.CreateMaskedLoad(Base[3], Mask[3], Q3);
  return buildPack(L0, L1, L2, L3);
}

void vectorizeUndefPassthruAliasMemoryTree(TreeEntry &Entry, Value **In,
                                           Value **Out, Value **Mask,
                                           IRBuilder &Builder,
                                           TargetTransformInfo &TTI) {
  if (allSameOpcode(Entry, Instruction::Add) && isValidElementType(Entry) &&
      noAlias(In, Out) && isProfitable(Entry, TTI)) {
    Value *LHS = loadUndefPassthruAliasMemory(In, Mask, Builder);
