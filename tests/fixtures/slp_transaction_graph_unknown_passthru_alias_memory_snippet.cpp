...
Value *loadUnknownPassthruAliasMemory(Value **Base, Value **Cmp,
                                      Value **Passthru, IRBuilder &Builder) {
  Value *M0 = Builder.CreateICmp(CmpInst::ICMP_EQ, Cmp[0], Passthru[0]);
  Value *P0 = unknownPassthru(Passthru[0]);
...
void vectorizeUnknownPassthruAliasMemoryTree(TreeEntry &Entry, Value **In,
                                             Value **Cmp, Value **Passthru,
                                             IRBuilder &Builder,
                                             TargetTransformInfo &TTI) {
  if (allSameOpcode(Entry, Instruction::Add) && isValidElementType(Entry) &&
      isProfitable(Entry, TTI)) {
    Value *LHS = loadUnknownPassthruAliasMemory(In, Cmp, Passthru, Builder);
