...
Value *loadUnknownAliasSymbolicMemory(Value **Base, int Lane,
                                      IRBuilder &Builder) {
...
void vectorizeSymbolicGatherStoreAliasUnknownTree(TreeEntry &Entry, Value **In,
                                                  Value **Out, int Lane,
                                                  IRBuilder &Builder,
                                                  TargetTransformInfo &TTI) {
  if (allSameOpcode(Entry, Instruction::Add) && isValidElementType(Entry) &&
      isProfitable(Entry, TTI)) {
    Value *LHS = loadUnknownAliasSymbolicMemory(In, Lane, Builder);
