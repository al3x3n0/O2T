...
void vectorizeWideningTruncReduction(TreeEntry &Entry, IRBuilder &Builder, TargetTransformInfo &TTI, LLVMContext &Ctx) {
  if (canVectorize(Entry) && isValidElementType(Entry) && isProfitable(Entry, TTI)) {
    ...
    Value *Narrow = Builder.CreateTrunc(Reduced);
    replaceScalarUses(Entry, Narrow);
  }
}
