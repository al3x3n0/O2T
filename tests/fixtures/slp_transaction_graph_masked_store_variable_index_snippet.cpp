...
Value *loadMaskedMemoryForVariableStore(Value **Base, Value **Mask,
                                        Value **Passthru,
                                        IRBuilder &Builder) {
...
void vectorizeMaskedVariableStoreSinkTree(TreeEntry &Entry, Value **In,
  ...
  if (allSameOpcode(Entry, Instruction::Add) && isValidElementType(Entry) &&
    ...
    Value *LHS = loadMaskedMemoryForVariableStore(In, Mask, Passthru, Builder);
    Value *RHS = packOperand(Entry, 1);
    Value *VectorAdd = Builder.CreateAdd(LHS, RHS);
    Value *VectorResult = Builder.CreateXor(VectorAdd, RHS);
    Builder.CreateMaskedStore(VectorResult, Out[I], Mask[0]);
