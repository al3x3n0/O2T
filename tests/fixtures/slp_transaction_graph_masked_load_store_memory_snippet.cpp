...
Value *loadMaskedMemory(Value **Base, Value **Mask, Value **Passthru,
                        IRBuilder &Builder) {
...
void vectorizeMaskedLoadStoreMemoryTree(TreeEntry &Entry, Value **In,
                                        Value **Out, Value **Mask,
                                        Value **Passthru, IRBuilder &Builder,
                                        TargetTransformInfo &TTI) {
  if (allSameOpcode(Entry, Instruction::Add) && isValidElementType(Entry) &&
      noAlias(In, Out) && isProfitable(Entry, TTI)) {
    Value *LHS = loadMaskedMemory(In, Mask, Passthru, Builder);
