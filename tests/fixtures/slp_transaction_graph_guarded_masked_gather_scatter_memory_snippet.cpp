...
Value *loadGuardedMaskedGather(Value **Base, Value **Mask, Value **Passthru,
                               IRBuilder &Builder) {
  Value *L0 = Passthru[0];
  if (Mask[0])
...
void vectorizeGuardedMaskedGatherScatterMemoryTree(TreeEntry &Entry,
                                                   Value **In, Value **Out,
                                                   Value **Mask,
                                                   Value **Passthru,
                                                   IRBuilder &Builder,
  ...
  if (allSameOpcode(Entry, Instruction::Add) && isValidElementType(Entry) &&
    ...
    Value *LHS = loadGuardedMaskedGather(In, Mask, Passthru, Builder);
