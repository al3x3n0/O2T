...
extern unsigned UnknownLane;

Value *loadUnknownMaskIndexMemory(Value **Base, Value **Mask,
                                  Value **Passthru, IRBuilder &Builder) {
  Value *L0 = Builder.CreateMaskedLoad(Base[0], Mask[UnknownLane], Passthru[0]);
...
void vectorizeUnknownMaskIndexMemoryTree(TreeEntry &Entry, Value **In,
                                         Value **Mask, Value **Passthru,
                                         IRBuilder &Builder,
                                         TargetTransformInfo &TTI) {
  if (allSameOpcode(Entry, Instruction::Add) && isValidElementType(Entry) &&
      isProfitable(Entry, TTI)) {
    Value *LHS = loadUnknownMaskIndexMemory(In, Mask, Passthru, Builder);
