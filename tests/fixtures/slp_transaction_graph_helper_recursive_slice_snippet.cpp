...
Value *recursiveMask(Value *L, Value *R, IRBuilder &Builder) {
  return recursiveMask(L, R, Builder);
}

Value *loadRecursiveMaskMemory(Value **Base, Value **Cmp, Value **Passthru,
                               IRBuilder &Builder) {
  Value *M0 = recursiveMask(Cmp[0], Passthru[0], Builder);
  Value *L0 = Builder.CreateMaskedLoad(Base[0], M0, Passthru[0]);
  Value *M1 = recursiveMask(Cmp[1], Passthru[1], Builder);
  Value *L1 = Builder.CreateMaskedLoad(Base[1], M1, Passthru[1]);
  Value *M2 = recursiveMask(Cmp[2], Passthru[2], Builder);
  Value *L2 = Builder.CreateMaskedLoad(Base[2], M2, Passthru[2]);
  Value *M3 = recursiveMask(Cmp[3], Passthru[3], Builder);
  Value *L3 = Builder.CreateMaskedLoad(Base[3], M3, Passthru[3]);
  return buildPack(L0, L1, L2, L3);
}

void vectorizeRecursiveHelperSliceTree(TreeEntry &Entry, Value **In,
                                       Value **Cmp, Value **Passthru,
                                       IRBuilder &Builder,
                                       TargetTransformInfo &TTI) {
  if (allSameOpcode(Entry, Instruction::Add) && isValidElementType(Entry) &&
      isProfitable(Entry, TTI)) {
    Value *LHS = loadRecursiveMaskMemory(In, Cmp, Passthru, Builder);
