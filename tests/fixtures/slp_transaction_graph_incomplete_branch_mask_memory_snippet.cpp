...
Value *loadIncompleteBranchMaskMemory(Value **Base, Value **A,
...
void vectorizeIncompleteBranchMaskMemoryTree(TreeEntry &Entry, Value **In,
                                             Value **A, Value **Passthru,
                                             IRBuilder &Builder,
                                             TargetTransformInfo &TTI) {
  if (allSameOpcode(Entry, Instruction::Add) && isValidElementType(Entry) &&
      isProfitable(Entry, TTI)) {
    Value *LHS = loadIncompleteBranchMaskMemory(In, A, Passthru, Builder);
