...
Value *loadSplitMaskAssignmentMemory(Value **Base, Value **A, Value **Passthru,
                                     IRBuilder &Builder) {
  Value *M0;
  M0 = Builder.CreateICmp(CmpInst::ICMP_EQ, A[0], Passthru[0]);
  Value *L0 = Builder.CreateMaskedLoad(Base[0], M0, Passthru[0]);
  Value *M1;
  M1 = Builder.CreateICmp(CmpInst::ICMP_EQ, A[1], Passthru[1]);
  Value *L1 = Builder.CreateMaskedLoad(Base[1], M1, Passthru[1]);
  Value *M2;
  M2 = Builder.CreateICmp(CmpInst::ICMP_EQ, A[2], Passthru[2]);
  Value *L2 = Builder.CreateMaskedLoad(Base[2], M2, Passthru[2]);
  Value *M3;
  M3 = Builder.CreateICmp(CmpInst::ICMP_EQ, A[3], Passthru[3]);
  Value *L3 = Builder.CreateMaskedLoad(Base[3], M3, Passthru[3]);
  return buildPack(L0, L1, L2, L3);
}

void vectorizeSplitMaskAssignmentMemoryTree(TreeEntry &Entry, Value **In,
                                            Value **Out, Value **A,
                                            Value **Passthru,
                                            IRBuilder &Builder,
  ...
  if (allSameOpcode(Entry, Instruction::Add) && isValidElementType(Entry) &&
    ...
    Value *LHS = loadSplitMaskAssignmentMemory(In, A, Passthru, Builder);
    Value *RHS = packOperand(Entry, 1);
    Value *VectorAdd = Builder.CreateAdd(LHS, RHS);
    Value *VectorResult = Builder.CreateXor(VectorAdd, RHS);
    Value *S0;
    S0 = Builder.CreateICmp(CmpInst::ICMP_EQ, A[0], Passthru[0]);
    Builder.CreateMaskedStore(VectorResult, Out[0], S0);
    Value *S1;
    S1 = Builder.CreateICmp(CmpInst::ICMP_EQ, A[1], Passthru[1]);
    Builder.CreateMaskedStore(VectorResult, Out[1], S1);
    Value *S2;
    S2 = Builder.CreateICmp(CmpInst::ICMP_EQ, A[2], Passthru[2]);
    Builder.CreateMaskedStore(VectorResult, Out[2], S2);
    Value *S3;
    S3 = Builder.CreateICmp(CmpInst::ICMP_EQ, A[3], Passthru[3]);
