...
Value *nullPassthru(Value *P) { return nullptr; }

Value *loadHelperUndefPassthruMemory(Value **Base, Value **Mask,
                                     Value **Passthru, IRBuilder &Builder) {
  Value *P0 = nullPassthru(Passthru[0]);
  Value *L0 = Builder.CreateMaskedLoad(Base[0], Mask[0], P0);
  Value *P1 = nullPassthru(Passthru[1]);
  Value *L1 = Builder.CreateMaskedLoad(Base[1], Mask[1], P1);
  Value *P2 = nullPassthru(Passthru[2]);
  Value *Q2 = P2;
  Value *L2 = Builder.CreateMaskedLoad(Base[2], Mask[2], Q2);
  Value *P3 = nullPassthru(Passthru[3]);
  Value *L3 = Builder.CreateMaskedLoad(Base[3], Mask[3], P3);
  return buildPack(L0, L1, L2, L3);
}

void vectorizeHelperUndefPassthruMemoryTree(TreeEntry &Entry, Value **In,
  ...
  if (allSameOpcode(Entry, Instruction::Add) && isValidElementType(Entry) &&
    ...
    Value *LHS = loadHelperUndefPassthruMemory(In, Mask, Passthru, Builder);
