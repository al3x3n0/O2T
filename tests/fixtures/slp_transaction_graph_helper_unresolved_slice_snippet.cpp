...
Value *unresolvedMask(Value *A) { return missingMaskBody(A); }

Value *loadUnresolvedHelperSliceMemory(Value **Base, Value **Cmp,
                                       Value **Passthru,
                                       IRBuilder &Builder) {
  Value *M0 = unresolvedMask(Cmp[0]);
  Value *L0 = Builder.CreateMaskedLoad(Base[0], M0, Passthru[0]);
  Value *M1 = unresolvedMask(Cmp[1]);
  Value *L1 = Builder.CreateMaskedLoad(Base[1], M1, Passthru[1]);
  Value *M2 = unresolvedMask(Cmp[2]);
  Value *L2 = Builder.CreateMaskedLoad(Base[2], M2, Passthru[2]);
  Value *M3 = unresolvedMask(Cmp[3]);
  Value *L3 = Builder.CreateMaskedLoad(Base[3], M3, Passthru[3]);
  return buildPack(L0, L1, L2, L3);
}

void vectorizeUnresolvedHelperSliceTree(TreeEntry &Entry, Value **In,
                                        Value **Cmp, Value **Passthru,
                                        IRBuilder &Builder,
                                        TargetTransformInfo &TTI) {
  if (allSameOpcode(Entry, Instruction::Add) && isValidElementType(Entry) &&
      isProfitable(Entry, TTI)) {
    Value *LHS = loadUnresolvedHelperSliceMemory(In, Cmp, Passthru, Builder);
