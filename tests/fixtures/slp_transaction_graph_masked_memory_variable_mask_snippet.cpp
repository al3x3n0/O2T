...
Value *loadVariableMaskedMemory(Value **Base, Value **Mask, Value **Passthru,
                                unsigned Lane, IRBuilder &Builder) {
  Value *L0 = Builder.CreateMaskedLoad(Base[0], Mask[Lane], Passthru[0]);
...
void vectorizeVariableMaskedMemoryTree(TreeEntry &Entry, Value **In,
  ...
  if (allSameOpcode(Entry, Instruction::Add) && isValidElementType(Entry) &&
    ...
    Value *LHS = loadVariableMaskedMemory(In, Mask, Passthru, Lane, Builder);
    Value *RHS = packOperand(Entry, 1);
    Value *VectorAdd = Builder.CreateAdd(LHS, RHS);
    Value *VectorResult = Builder.CreateXor(VectorAdd, RHS);
    Builder.CreateMaskedStore(VectorResult, Out[0], Mask[Lane]);
