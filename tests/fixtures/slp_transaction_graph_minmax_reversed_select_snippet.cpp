...
void vectorizeReversedMinGraph(TreeEntry &Entry, IRBuilder &Builder, TargetTransformInfo &TTI) {
  if (canVectorize(Entry) && isValidElementType(Entry) && isProfitable(Entry, TTI)) {
    ...
    Value *Selected = Builder.CreateSelect(Cmp, Extra, AddTmp);
    replaceScalarUses(Entry, Selected);
  }
}
