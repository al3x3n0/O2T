...
Value *loadPointerMutationPack(Value **Base, IRBuilder &Builder) {
...
void vectorizeMemoryPointerMutationTree(TreeEntry &Entry, Value **A,
                                        IRBuilder &Builder,
                                        TargetTransformInfo &TTI) {
  if (allSameOpcode(Entry, Instruction::Add) && isValidElementType(Entry) &&
      isProfitable(Entry, TTI)) {
    Value *LHS = loadPointerMutationPack(A, Builder);
