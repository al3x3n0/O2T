...
Value *loadScalableSymbolicMaskIndexPack(Value **Base, Value **Mask,
                                         Value **Passthru, unsigned Lane,
                                         IRBuilder &Builder) {
  Value *L0 = Builder.CreateMaskedLoad(Base[0], Mask[Lane + 1], Passthru[0]);
...
void vectorizeScalableSymbolicMaskIndexMemoryTree(
  ...
  if (allSameOpcode(Entry, Instruction::Add) && isValidElementType(Entry) &&
    ...
    Value *LHS =
        loadScalableSymbolicMaskIndexPack(In, Mask, Passthru, Lane, Builder);
    Value *RHS = packOperand(Entry, 1);
    Value *VectorAdd = Builder.CreateAdd(LHS, RHS);
    Value *VectorResult = Builder.CreateXor(VectorAdd, RHS);
    Builder.CreateMaskedStore(VectorResult, Out[0], Mask[Lane + 1]);
