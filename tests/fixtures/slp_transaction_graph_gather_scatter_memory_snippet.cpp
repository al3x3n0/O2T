...
Value *loadGatherMemory(Value **Base, IRBuilder &Builder) {
...
void vectorizeGatherScatterMemoryTree(TreeEntry &Entry, Value **In, Value **Out,
                                      IRBuilder &Builder,
                                      TargetTransformInfo &TTI) {
  if (allSameOpcode(Entry, Instruction::Add) && isValidElementType(Entry) &&
      noAlias(In, Out) &&
      isProfitable(Entry, TTI)) {
    Value *LHS = loadGatherMemory(In, Builder);
