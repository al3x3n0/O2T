...
Value *loadImplicitUndefPassthruMemory(Value **Base, Value **Mask,
                                       IRBuilder &Builder) {
...
void vectorizeImplicitUndefPassthruMemoryTree(TreeEntry &Entry, Value **In,
                                              Value **Out, Value **Mask,
                                              IRBuilder &Builder,
                                              TargetTransformInfo &TTI) {
  if (allSameOpcode(Entry, Instruction::Add) && isValidElementType(Entry) &&
      noAlias(In, Out) && isProfitable(Entry, TTI)) {
    Value *LHS = loadImplicitUndefPassthruMemory(In, Mask, Builder);
