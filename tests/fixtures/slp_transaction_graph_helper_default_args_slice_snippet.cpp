...
Value *defaultedMask(Value *L, IRBuilder &Builder, Value *R = 0) {
  return Builder.CreateICmp(CmpInst::ICMP_EQ, L, R);
}

Value *loadDefaultArgsHelperSliceMemory(Value **Base, Value **Cmp,
                                        Value **Passthru,
                                        IRBuilder &Builder) {
  Value *M0 = defaultedMask(Cmp[0], Builder);
  Value *L0 = Builder.CreateMaskedLoad(Base[0], M0, Passthru[0]);
  Value *M1 = defaultedMask(Cmp[1], Builder);
  Value *L1 = Builder.CreateMaskedLoad(Base[1], M1, Passthru[1]);
  Value *M2 = defaultedMask(Cmp[2], Builder);
  Value *L2 = Builder.CreateMaskedLoad(Base[2], M2, Passthru[2]);
  Value *M3 = defaultedMask(Cmp[3], Builder);
  Value *L3 = Builder.CreateMaskedLoad(Base[3], M3, Passthru[3]);
  return buildPack(L0, L1, L2, L3);
}

void vectorizeDefaultArgsHelperSliceTree(TreeEntry &Entry, Value **In,
                                         Value **Cmp, Value **Passthru,
                                         IRBuilder &Builder,
                                         TargetTransformInfo &TTI) {
  if (allSameOpcode(Entry, Instruction::Add) && isValidElementType(Entry) &&
      isProfitable(Entry, TTI)) {
    Value *LHS = loadDefaultArgsHelperSliceMemory(In, Cmp, Passthru, Builder);
