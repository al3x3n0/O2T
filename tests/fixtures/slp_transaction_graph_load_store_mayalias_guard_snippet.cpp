...
Value *loadMayAliasGuardMemory(Value **Base, IRBuilder &Builder) {
...
void vectorizeMayAliasGuardMemoryTree(TreeEntry &Entry, Value **In, Value **Out,
                                      IRBuilder &Builder,
                                      TargetTransformInfo &TTI) {
  if (allSameOpcode(Entry, Instruction::Add) && isValidElementType(Entry) &&
      !mayAlias(In, Out) && isProfitable(Entry, TTI)) {
    Value *LHS = loadMayAliasGuardMemory(In, Builder);
